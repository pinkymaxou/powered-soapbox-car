...
void ControllerBase::updateLVC(float vbat, int64_t now)
{
...
float ControllerBase::brakeWheel(Pid& pid, float speed_ms, const KartConfig& cfg, float dt)
{
...
void ControllerBase::updateEncStuck(int64_t now, float cmd, float speed_ms)
{
...
void ControllerBase::updateEncSanity(int64_t now, bool braking, float out_l, float out_r,
                                     float sl, float sr)
{
...
void ControllerBase::tick(const KartConfig& cfg)
{
    const int64_t now = ioNowUs();

    const bool use_enc = (cfg.use_encoders != 0.f);   // 0 = ignore les AS5600 (banc sans encodeurs)

    const CtrlPad in = ioPad();
    ...
    const bool pad_stale = in.connected &&
                           ((now - ioPadLastReportUs()) > hw::PAD_HB_TIMEOUT_US);
    ...
    const float vbat = vbat_valid ? m_vraw * cfg.vbat_div_ratio : 0.f;
    const float sl_raw = use_enc ? countsToMps(ioEncDeltaL()) : 0.f;
    const float sr_raw = use_enc ? countsToMps(ioEncDeltaR()) : 0.f;
    ...
    const bool enc_l_abs = use_enc && !ioEncPresentL();
    const bool enc_r_abs = use_enc && !ioEncPresentR();
    ...
    else if (in.connected && !ioPadCalibrated())    fault = Fault::NotCalibrated;  // manette non calibrée
    ...
    const bool calibrated = ioPadCalibrated();
    unsigned fmask = 0;
    if (in.estop)                                fmask |= fb::ESTOP;
    if (m_lvc_tripped)                           fmask |= fb::LVC;
    if (in.connected && !calibrated)             fmask |= fb::NOCAL;
    ...
    const bool btn_start_hw = ioBtnStart();
    const bool start_held = btn_start_hw || in.start;
    const bool centered = (std::fabs(in.x) < hw::ARM_CENTER_MAX) && (std::fabs(in.y) < hw::ARM_CENTER_MAX);
    if (start_held)
    {
    ...
    if ((hard_fault && !m_hardfault_prev) || (in.estop && !m_estop_prev))  // erreur soudaine / e-stop → fort
        ioRumble(255, 255, 450);
    const bool pushing = (std::fabs(in.rx) > hw::PUSH_MIN) || (std::fabs(in.ry) > hw::PUSH_MIN);
    if (pushing && !can_drive && (now - m_rumble_block_us) > hw::RUMBLE_BLOCK_INTERVAL_US)   // bouge mais bloqué → fort (répété)
    {
        ioRumble(220, 220, 250);
        m_rumble_block_us = now;
    }
    if (m_armed_prev && !m_armed) ioFlushPendingConfig();   // set reçu en roulant → persisté ici
    ...
    if (!can_drive)
    {
        ...
        m_turn_cmd = 0.f;
        state = (Fault::None != fault) ? State::Fault : State::Lockout;
    }
    else
    {
        m_brake_l.reset();
        ...
        {
            const float turn_max = turnLimit(std::fabs(v_veh), cfg.turn_full_ms, cfg.speed_limit_ms, cfg.turn_hi);
        ...
        {
            const float vcap = m_speed_pid.update(cfg.speed_limit_ms, std::fabs(v_veh), hw::CTRL_DT_S,
    ...
    if (dyn_brake) ioMotorsBrake();                  // court-circuit moteur (freinage dynamique)
    else           ioMotorsSet(out_l, out_r, cap);   // pilotage / plugging actif
    if (use_enc)
    {
    ...
    m_tel.armed      = m_armed;
    m_tel.btn_start  = btn_start_hw || in.start;
}
