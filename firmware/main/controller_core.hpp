...
//   1. fill the two callbacks (setCallbacks): updateSensors → read the encoders,
...
// SENSOR reading (the two wheel encoders): data AND read errors — this is the
...
struct CtrlInputs
{
    ...
    SensorReadings sensors;          // wheel encoders (see setCallbacks)
};
